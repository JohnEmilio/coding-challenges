...
function isPrime(num) { // To see if the current number is a prime
...
function findEmirp(n) {
    ...
    while (n > 12) { // Since 13 is the lowest prime integer 
        if (isPrime(n)) { //This will see if the current n is a prime by using the isPrime function
            let temp = Number(String(n).split('').reverse().join('')) // This is the opposite of the current number
            if (temp != n) { // As long as the number is not a palindrome we continue
                if (isPrime(temp)) { // If the reverse number is also a prime, we push n into our primes array and add n to sum
                    primes.push(n)
                    sum += n
                }
            }
    ...
    return [primes.length, primes[0], sum]
}